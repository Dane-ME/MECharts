...
namespace MEGraph.MAUI.Series.Area
{
    public class AreaSeries : ISeries
    {
        ...
        // START - 2.4.0 - ADD - Gradient fill support
        public bool UseGradientFill { get; set; } = false;
        public Paint? FillGradient { get; set; }
        // END - 2.4.0 - ADD
        ...
        public void Draw(ICanvas canvas, RectF plotArea, float? globalMinY, float? globalMaxY, float progress)
        {
            ...
            if (IsSmooth && points.Length > 2)
            {
                ...
                for (int i = 0; i < n - 1; i++)
                {
                    ...
                    linePath.CurveTo(cp1x, cp1y, cp2x, cp2y, points[i + 1].X, points[i + 1].Y);
                }
            }
            else
            {
            ...
            if (UseGradientFill)
            {
                ...
                float seriesMinY = Data.Min();
                float topY = plotArea.Bottom - ((seriesMaxY - minY) / rangeY * plotArea.Height);
                float bottomY = plotArea.Bottom - ((seriesMinY - minY) / rangeY * plotArea.Height);
                if (bottomY <= topY) bottomY = plotArea.Bottom;

                var bounds = new RectF(plotArea.Left, topY, plotArea.Width, Math.Max(1f, bottomY - topY));

                var paint = FillGradient ?? new LinearGradientPaint
                {
                    StartPoint = new Point(0, 0),
                    EndPoint = new Point(0, 1),
                    GradientStops = new[]
                    {
                        new PaintGradientStop(0.0f, StrokeColor.WithAlpha(0.75f)),
                        new PaintGradientStop(1.0f, StrokeColor.WithAlpha(0.05f))
                    }
                };
